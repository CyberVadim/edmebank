...
import com.edmebank.clientmanagement.exception.InvalidAddressException;
...
public class AddressValidationServiceImpl implements AddressValidationService{
    ...
    public AddressValidationResultDto validate(String rawAddress) {
        ...
        if (parsed == null) {
            throw new InvalidAddressException("Failed to parse address: " + rawAddress);
        }
        if (parsed.coef < 80) {
            log.warn("Address parsing returned low confidence: {}", parsed.coef);
        }
        try {
            return toResult(parsed);
        } catch (Exception e) {
            log.error("Failed to convert address: {}", e.getMessage(), e);
            throw new InvalidAddressException("Address conversion failed", e);
        }
    }
